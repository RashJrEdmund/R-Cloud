...
const TextTag = styled.span<Props>`
  ...
  font-weight: ${({ weight = "400" }) =>
    weight}; // 400 is default and normal font-weight, 700 is bold
  font-size: ${({ size = "1rem" }) =>
    size}; // 1rem is default text size and 2rem is for like h1s
  color: ${({ color_type = "inherit" }) => generateColor(color_type)};
  white-space: ${({ no_white_space = false, ellipsis = false }) =>
    no_white_space || ellipsis ? "nowrap" : "normal"};
  ...
  @media only screen and (max-width: 900px) {
    color: ${({ media_color, color_type }) =>
      generateColor(media_color ?? color_type ?? "normal")};
    font-size: ${({ media_size, size }) =>
      media_size ??
      size ??

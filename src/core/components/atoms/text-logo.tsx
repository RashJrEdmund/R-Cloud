...
export default function TextLogo({ showLogo, sxImgClassName = "", sxTextClassName = "", className = "", ...restProps }: TextLogoProps) {
  return (
    <span
      {...restProps}
      className={cn("flex flex-nowrap items-center justify-center gap-1 text-app_text_grayed w-fit", className)}
    >
      {showLogo ? (
        <Image
          src={APP_ICONS.logo}

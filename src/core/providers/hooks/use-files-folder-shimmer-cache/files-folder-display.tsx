...
export default function useFilesFolderShimmerCache() {
  const [cardCount, setCardCount] = useState<number>(16); // should display 16 cards by default if current folder is not yet cached

  const pathname = usePathname();
  const params = useParams<{ folder_id: string }>();

  const { documents, currentFolder, setCurrentFolder } = useDocStore();

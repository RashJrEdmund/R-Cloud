...
type IHandleCreateUserProfile = (
  ...
  set_state_actions: {
    setFormStatus: React.Dispatch<React.SetStateAction<{ status: number, message: string } | null>>;
  }
) => Promise<void>;

const handleCreateUserProfile: IHandleCreateUserProfile = async (user, extra_data, { setFormStatus }) => {
  /* FUNC_DESC +=> ===========================================
